...
import {
  Home, Search, Bookmark, Calendar, Bell, User, Plus, Settings,
} from "lucide-react";
  ...
  return (
    <div className="min-h-dvh bg-app">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-line bg-surface lg:flex">
        <div className="flex h-16 items-center px-6"><Brand size="md" /></div>
        ...
        <nav className="flex-1 space-y-6 overflow-y-auto px-4 py-4">
          ...
          <div className="space-y-1">
            <p className="px-3 pb-1 text-xs font-semibold uppercase tracking-wider text-fg-muted">You</p>
          ...
          {mobileNav.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href}
                className={cn("flex flex-col items-center gap-0.5 rounded-lg px-2 py-1.5 transition",
                  active ? "text-brand" : "text-fg-tertiary")}>
                <Icon className={cn("size-5", active && "stroke-[2.5]")} strokeWidth={active ? 2.5 : 2} />
  ...
  return (
    <Link href={href}
      className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition",
        active ? "bg-brand-soft text-brand" : "text-fg-secondary hover:bg-surface-2 hover:text-fg")}>
