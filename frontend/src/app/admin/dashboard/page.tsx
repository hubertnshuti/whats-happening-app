...
import { adminService, StatsResponse } from '@/features/admin/service';
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    // 1. Safe check with optional chaining (?.)
    const hasAdminRole = user?.role?.includes('ADMIN') || user?.role?.includes('SUPER_ADMIN');
    
    // 2. Failsafe: If your DB is acting up, uncomment the line below and put your email in it to FORCE admin access for the demo!
    // const hasAdminRole = true; // user?.email === 'your-email@example.com';
